...
### Why the output differs each run

There are **two LLM calls** with two different settings:

| Call | Setting | Result |
|------|---------|--------|
| Agent LLM — picks the next tool | `ChatOllama(temperature=0)` | Same 4 tools, same order, every run |
| RCA LLM — writes the analysis | no `options` → Ollama default `0.8` | Different text every run |

**Temperature** controls how strictly the model follows its own ranking of the next word.
At `0` it always takes the top-ranked token. At `0.8` it samples, so lower-ranked tokens
get picked sometimes.

> **Temperature 0 doesn't make the model more correct. It makes it more consistent.**
> It returns whatever the model thinks is most probable. When the most probable answer
> is wrong — as with `affected_file`, where it reaches for a path from the log instead of
> reasoning that dependency constraints live in `package.json` — temperature 0 returns
> that same wrong answer reliably, instead of occasionally stumbling onto the right one.
