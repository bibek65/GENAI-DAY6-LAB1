...
# Lab 2 (Bonus) — Break the Reliability Controls ⏱️ 15 min

This lab shows what the safeguards are actually holding back.

**Work on a copy so your working agent stays intact:**

```bash
cp ~/day6/lab1_ci_agent.py ~/day6/lab2_broken.py
```

---

## Step 2.1 — Remove the code guard only

In `lab2_broken.py`, comment out the one-tool limit inside `agent_node`:
...
```bash
python3 ~/day6/lab2_broken.py
```

### What happens: nothing breaks
...
Still one tool per turn, still the correct order.

**Why?** Because the prompt is also enforcing it:

```
- Call exactly ONE tool per turn, then wait for its result.
```

You have **two independent layers of defence** — the prompt and the code — and the
prompt is silently carrying the load. Removing one control did not expose the problem.

> **Teaching point:** you cannot tell which safeguard is doing the work until you remove
> them one at a time. A system that still works after you delete a guardrail may just
> mean a *different* guardrail caught it.

---

## Step 2.2 — Remove the prompt rule as well

Now also replace the two-message prompt with a single loose instruction, the way you
would naturally write it first:

```python
    initial_prompt = """
You are a CI failure analysis agent.

Your job is to investigate a CI failure and
prepare a proposed GitHub PR description.

Follow these steps:

1. Read the CI failure log using read_log_file.
2. Search the knowledge base using search_knowledge_base.
3. Analyze the root cause using analyze_root_cause.
4. Generate a PR description using draft_pr_description.

Important rules:

- Use one tool at a time.
- Do not skip the investigation steps.
- Use information returned by previous tools.
...
Note this prompt *does* say "Use one tool at a time." The model ignores it.
...
## Step 2.3 — Watch the failure cascade
...
🧠 LLM requested tool:
   → read_log_file
     Arguments: {'filepath': '/path/to/CI/failure/log.txt'}
   → search_knowledge_base
     Arguments: {'query': 'CI failure with similar symptoms'}

🔧 TOOL CALL: read_log_file
🔧 TOOL CALL: search_knowledge_base
   query = CI failure with similar symptoms
   ✅ Found 1 similar failure(s)
   ↩︎  search_knowledge_base: [similarity: 0.61] [database] Integration tests failed in CI:
      connection refused to postgres on localhost port 5432 …

🧠 LLM requested tool:
   → analyze_root_cause
   🩹 Normalized arguments: {'root_cause': 'connection refused to postgres on localhost port 5432'}
      → {'log_content': 'connection refused to postgres on localhost port 5432'}
```
...
"postgres": "13.5", "github-actions/service-container": "v2.0.3" - ECONNREFUSED: Connection refused
...
# No fix command generated
```

**There is no database in this pipeline.** The real failure — npm ERESOLVE, react 18 vs
react 17 — is never mentioned.

### The causal chain

Trace how one removed control produced a completely wrong document:

```
1. Two tools fired in the same turn
        ↓
2. search_knowledge_base ran BEFORE read_log_file returned
        ↓
3. With no log to quote, the model invented a generic query:
   "CI failure with similar symptoms"
        ↓
4. A generic query matched a generic entry — postgres, 0.61 similarity
        ↓
5. analyze_root_cause anchored on postgres instead of npm
        ↓
6. The PR describes a database outage that never happened
```

The model never lied about a single step. Each step was *reasonable given the previous
one*. The failure was **structural** — the workflow let step 2 run before step 1 finished.

Also visible in that run: `read_log_file` was called with the invented path
`/path/to/CI/failure/log.txt`, and `analyze_root_cause` was called with the invented
parameter `root_cause`. The **path fallback** and **argument normalization** controls
both fired and quietly repaired those — which is why the run completed at all instead of
crashing.

### What this teaches

The model is not being stupid. It is being **probabilistic**: asked to do four things, it
predicts all four tool calls in one pass because nothing in the token stream forces it
to wait.

> **Waiting is a property of your workflow, not of the model.**
> A prompt *requests* sequencing. Code *enforces* it.
> Only one of those is reliable with a 3B model.

Restore the guard, or just delete `lab2_broken.py` and keep using `lab1_ci_agent.py`.
